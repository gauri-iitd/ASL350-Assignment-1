...
[depth_grid, lat_grid] = meshgrid(depth, lat(lat_indices));

figure;
surf(depth_grid, lat_grid, squeeze(salinity_slopes(:, 1, :))');
title('Salinity Slope vs. Depth for the First Latitude');
xlabel('Depth (meters)');
ylabel('Latitude');
zlabel('Slope');

figure;
surf(depth_grid, lat_grid, squeeze(temperature_slopes(:, 1, :))');
title('Temperature Slope vs. Depth for the First Latitude');
xlabel('Depth (meters)');
ylabel('Latitude');
zlabel('Slope');
